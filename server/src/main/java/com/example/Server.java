        ...
        public void comunica () throws Exception 
        {
            ...
            else 
            {
                for(int i = 0; i < utenti.size(); i++)
                {
                    ...
                    {
                        outVersoClient.writeBytes("Nome utente già utilizzato, provare con uno nuovo" + '\n');
                        usernameClient = inDalClient.readLine();
                    }
                }
            ...
            {
                sockets.get(i).outVersoClient.writeBytes("Utenti online:" + '\n');
            ...
            for(;;) 
            {
                ...
                {
                    System.out.println(usernameClient + ": utente offline" + '\n');
                ...
                else 
                {
                    ...
                    if(sockets.size() > 1) 
                    {
                        if(mexSplit[1].equalsIgnoreCase("Tutti")) 
                        {         
                            for (ServerThread s : sockets) 
                            {
                                if(s != this) s.outVersoClient.writeBytes(usernameClient + ": " + mexSplit[0] + ("(Nel gruppo)") + '\n');
                            }
                        }
                        else 
                        {
